...
class Model(tf.keras.Model):
    ...
    def call(self, src: tf.Tensor, trg_input: tf.Tensor, src_mask: tf.Tensor,src_lengths: tf.Tensor, trg_mask: tf.Tensor = None, src_input: tf.Tensor = None): 
        """
        ...
        #     noise[:,:,-1] = tf.zeros_like(noise[:, :, -1])

        #     # Need to add a zero on the end of
        #     if self.future_prediction != 0:
        #         self.out_stds = tf.concat((self.out_stds,tf.zeros_like(self.out_stds)))[:trg_input.shape[-1]]
...
def build_model(cfg: dict = None,
    ...
    trg_linear = tf.keras.layers.Dense(cfg["decoder"]["embeddings"]["embedding_dim"])
